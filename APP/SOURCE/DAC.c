...
{ 
  ...
  DAC_InitStructure.DAC_Trigger = DAC_Trigger_T2_TRGO; //Set the laser current to use the timer to trigger
  ...
  DAC_InitStructure.DAC_Trigger = DAC_Trigger_Software; //Set the laser temperature without triggering
  DAC_InitStructure.DAC_WaveGeneration = DAC_WaveGeneration_None;
  DAC_InitStructure.DAC_OutputBuffer = DAC_OutputBuffer_Disable;
...
void TIM2_Configuration(void)
{
  ...
  TIM_TimeBaseStructure.TIM_Period = 0x1C1;     //72000000/(TIM_Prescaler+1)/(TIM_Period+1)=10000 10000 period/ps
  TIM_TimeBaseStructure.TIM_Prescaler = 0xF;    //Every ADC point converting time is 0.0001s Period=0.0001s*1000=0.1=10HZ
